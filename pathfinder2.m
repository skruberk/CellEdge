...
% nxm image inside a larger,(n+2k)x(m+2k), image
%orgsize = length(inbwimage)
orgsize=size(inbwimage)
% size of input image
padsize(1) = 2*maxrad+orgsize(1);
padsize(2)=  2*maxrad+orgsize(2);
% size of padded image
bwimage = zeros(padsize(1),padsize(2));   % initialize padded image
% load original image into padded holder
bwimage(maxrad+1:maxrad+orgsize(1),maxrad+1:maxrad+orgsize(2))=inbwimage;
